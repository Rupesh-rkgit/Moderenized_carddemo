...
/**
 * Reference the spring application of program COACTUPC.
 *  Provides an access to the contained program for the run unit.
 */
@Component
@Import({
	aws.bluage.l3.workshop.coactupc.business.context.CoactupcConfiguration.class,
	aws.bluage.l3.workshop.coactupc.business.context.CoactupcContext.class,
	aws.bluage.l3.workshop.coactupc.service.impl.CoactupcProcessImpl.class
})
public class Coactupc implements Program {
	/**
	 * Unique identifiers for the contained program.
	 */
	private static final Set<String> programIdentifiers = Collections.unmodifiableSet(Stream.of("COACTUPC").collect(Collectors.toSet()));

	/**
	 * Main program identifier for the contained program.
	 */
	private static final String programIdentifier = "COACTUPC";
	@Autowired
	...
	public void run(ExecutionController ctrl) {
		instanceCoactupcProcess.coactupc((CoactupcContext) getContext(), ctrl);
	}	
