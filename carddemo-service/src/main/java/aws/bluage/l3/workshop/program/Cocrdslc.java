...
/**
 * Reference the spring application of program COCRDSLC.
 *  Provides an access to the contained program for the run unit.
 */
@Component
@Import({
	aws.bluage.l3.workshop.cocrdslc.business.context.CocrdslcConfiguration.class,
	aws.bluage.l3.workshop.cocrdslc.business.context.CocrdslcContext.class,
	aws.bluage.l3.workshop.cocrdslc.service.impl.CocrdslcProcessImpl.class
})
public class Cocrdslc implements Program {
	/**
	 * Unique identifiers for the contained program.
	 */
	private static final Set<String> programIdentifiers = Collections.unmodifiableSet(Stream.of("COCRDSLC").collect(Collectors.toSet()));

	/**
	 * Main program identifier for the contained program.
	 */
	private static final String programIdentifier = "COCRDSLC";
	@Autowired
	...
	public void run(ExecutionController ctrl) {
		instanceCocrdslcProcess.cocrdslc((CocrdslcContext) getContext(), ctrl);
	}	
