...
import aws.bluage.l3.workshop.coactupc.statemachine.CoactupcProcedureDivisionStateMachineController.States;
import aws.bluage.l3.workshop.coactupc.statemachine.CoactupcProcedureDivisionStateMachineService;
 ...
 */
@Component("aws.bluage.l3.workshop.coactupc.statemachine.CoactupcProcedureDivisionStateMachineController")
@Import({
aws.bluage.l3.workshop.coactupc.statemachine.CoactupcProcedureDivisionStateMachineService.class
})
@Lazy
public class CoactupcProcedureDivisionStateMachineController extends MultipleActiveContinuationController<States, Events> {
	...
	protected void configureStateMachine(StateMachineStateConfigurer<States, Events> states, StateMachineTransitionConfigurer<States, Events> transitions, RuntimeContext ctx, ExecutionController ctrl) throws Exception {
		...
		subConfigurer.state(States._9600_WRITE_PROCESSING, buildAction(() -> {stateProcess._9600WriteProcessing(lctx, ctrl);}), null);
		subConfigurer.state(States._9600_WRITE_PROCESSING_EXIT, buildAction(() -> {stateProcess._9600WriteProcessingExit(lctx, ctrl);}), null);
		subConfigurer.state(States._9700_CHECK_CHANGE_IN_REC, buildAction(() -> {stateProcess._9700CheckChangeInRec(lctx, ctrl);}), null);
		subConfigurer.state(States._9700_CHECK_CHANGE_IN_REC_EXIT, buildAction(() -> {stateProcess._9700CheckChangeInRecExit(lctx, ctrl);}), null);
		...
		subConfigurer.state(States._2000_DECIDE_ACTION_POST__9600WRITEPROCESSING_THRU__9600WRITEPROCESSINGEXIT, buildAction(() -> {stateProcess._2000DecideActionPost9600writeprocessingThru9600writeprocessingexit(lctx, ctrl);}), null);
		subConfigurer.state(States._9600_WRITE_PROCESSING_POST__9700CHECKCHANGEINREC_THRU__9700CHECKCHANGEINRECEXIT, buildAction(() -> {stateProcess._9600WriteProcessingPost9700checkchangeinrecThru9700checkchangeinrecexit(lctx, ctrl);}), null);
	...
	private void configureTransitions(StateMachineTransitionConfigurer<States, Events> transitions) throws Exception {
		...
		transitions.withExternal().source(States._9600_WRITE_PROCESSING).target(States._9600_WRITE_PROCESSING_EXIT).event(Events.TO__9600_WRITE_PROCESSING_EXIT);
		transitions.withExternal().source(States._9600_WRITE_PROCESSING).target(States._9700_CHECK_CHANGE_IN_REC).event(Events.TO__9700_CHECK_CHANGE_IN_REC);
		transitions.withExternal().source(States._9600_WRITE_PROCESSING_EXIT).target(States._2000_DECIDE_ACTION_POST__9600WRITEPROCESSING_THRU__9600WRITEPROCESSINGEXIT).event(Events.TO__2000_DECIDE_ACTION_POST__9600WRITEPROCESSING_THRU__9600WRITEPROCESSINGEXIT);
		transitions.withExternal().source(States._9700_CHECK_CHANGE_IN_REC).target(States._9600_WRITE_PROCESSING_EXIT).event(Events.TO__9600_WRITE_PROCESSING_EXIT);
		transitions.withExternal().source(States._9700_CHECK_CHANGE_IN_REC).target(States._9700_CHECK_CHANGE_IN_REC_EXIT).event(Events.TO__9700_CHECK_CHANGE_IN_REC_EXIT);
		transitions.withExternal().source(States._9700_CHECK_CHANGE_IN_REC_EXIT).target(States._9600_WRITE_PROCESSING_POST__9700CHECKCHANGEINREC_THRU__9700CHECKCHANGEINRECEXIT).event(Events.TO__9600_WRITE_PROCESSING_POST__9700CHECKCHANGEINREC_THRU__9700CHECKCHANGEINRECEXIT);
		...
		transitions.withExternal().source(States._2000_DECIDE_ACTION_CASEBRANCH).target(States._9600_WRITE_PROCESSING).event(Events.TO__9600_WRITE_PROCESSING);
		...
		transitions.withExternal().source(States._2000_DECIDE_ACTION_POST__9600WRITEPROCESSING_THRU__9600WRITEPROCESSINGEXIT).target(States._2000_DECIDE_ACTION_POSTIF).event(Events.TO__2000_DECIDE_ACTION_POSTIF);
		transitions.withExternal().source(States._9600_WRITE_PROCESSING_POST__9700CHECKCHANGEINREC_THRU__9700CHECKCHANGEINRECEXIT).target(States._9600_WRITE_PROCESSING_EXIT).event(Events.TO__9600_WRITE_PROCESSING_EXIT);
	}
}
