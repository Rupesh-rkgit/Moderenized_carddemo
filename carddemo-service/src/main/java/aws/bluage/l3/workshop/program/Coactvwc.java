...
/**
 * Reference the spring application of program COACTVWC.
 *  Provides an access to the contained program for the run unit.
 */
@Component
@Import({
	aws.bluage.l3.workshop.coactvwc.business.context.CoactvwcConfiguration.class,
	aws.bluage.l3.workshop.coactvwc.business.context.CoactvwcContext.class,
	aws.bluage.l3.workshop.coactvwc.service.impl.CoactvwcProcessImpl.class
})
public class Coactvwc implements Program {
	/**
	 * Unique identifiers for the contained program.
	 */
	private static final Set<String> programIdentifiers = Collections.unmodifiableSet(Stream.of("COACTVWC").collect(Collectors.toSet()));

	/**
	 * Main program identifier for the contained program.
	 */
	private static final String programIdentifier = "COACTVWC";
	@Autowired
	...
	public void run(ExecutionController ctrl) {
		instanceCoactvwcProcess.coactvwc((CoactvwcContext) getContext(), ctrl);
	}	
