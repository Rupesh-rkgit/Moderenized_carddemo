...
/**
 * Reference the spring application of program COADM01C.
 *  Provides an access to the contained program for the run unit.
 */
@Component
@Import({
	aws.bluage.l3.workshop.coadm01c.business.context.Coadm01cConfiguration.class,
	aws.bluage.l3.workshop.coadm01c.business.context.Coadm01cContext.class,
	aws.bluage.l3.workshop.coadm01c.service.impl.Coadm01cProcessImpl.class
})
public class Coadm01c implements Program {
	/**
	 * Unique identifiers for the contained program.
	 */
	private static final Set<String> programIdentifiers = Collections.unmodifiableSet(Stream.of("COADM01C").collect(Collectors.toSet()));

	/**
	 * Main program identifier for the contained program.
	 */
	private static final String programIdentifier = "COADM01C";
	@Autowired
	 ...
	 */
	@Autowired
	@Qualifier("aws.bluage.l3.workshop.coadm01c.service.Coadm01cProcess")
	private Coadm01cProcess instanceCoadm01cProcess;
	
	@Override
	public void run(ExecutionController ctrl) {
		instanceCoadm01cProcess.mainPara((Coadm01cContext) getContext(), ctrl);
	}	
