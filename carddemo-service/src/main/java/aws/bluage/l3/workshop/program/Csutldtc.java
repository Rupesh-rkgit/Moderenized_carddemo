...
/**
 * Reference the spring application of program CSUTLDTC.
 *  Provides an access to the contained program for the run unit.
 */
@Component
@Import({
	aws.bluage.l3.workshop.csutldtc.business.context.CsutldtcConfiguration.class,
	aws.bluage.l3.workshop.csutldtc.business.context.CsutldtcContext.class,
	aws.bluage.l3.workshop.csutldtc.service.impl.CsutldtcProcessImpl.class
})
public class Csutldtc implements Program {
	/**
	 * Unique identifiers for the contained program.
	 */
	private static final Set<String> programIdentifiers = Collections.unmodifiableSet(Stream.of("CSUTLDTC").collect(Collectors.toSet()));

	/**
	 * Main program identifier for the contained program.
	 */
	private static final String programIdentifier = "CSUTLDTC";
	@Autowired
	 ...
	 */
	@Autowired
	@Qualifier("aws.bluage.l3.workshop.csutldtc.service.CsutldtcProcess")
	private CsutldtcProcess instanceCsutldtcProcess;
	
	@Override
	public void run(ExecutionController ctrl) {
		instanceCsutldtcProcess.procedureDivision((CsutldtcContext) getContext(), ctrl);
	}	
