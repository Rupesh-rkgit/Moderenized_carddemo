...
/**
 * Reference the spring application of program COUSR01C.
 *  Provides an access to the contained program for the run unit.
 */
@Component
@Import({
	aws.bluage.l3.workshop.cousr01c.business.context.Cousr01cConfiguration.class,
	aws.bluage.l3.workshop.cousr01c.business.context.Cousr01cContext.class,
	aws.bluage.l3.workshop.cousr01c.service.impl.Cousr01cProcessImpl.class
})
public class Cousr01c implements Program {
	/**
	 * Unique identifiers for the contained program.
	 */
	private static final Set<String> programIdentifiers = Collections.unmodifiableSet(Stream.of("COUSR01C").collect(Collectors.toSet()));

	/**
	 * Main program identifier for the contained program.
	 */
	private static final String programIdentifier = "COUSR01C";
	@Autowired
	 ...
	 */
	@Autowired
	@Qualifier("aws.bluage.l3.workshop.cousr01c.service.Cousr01cProcess")
	private Cousr01cProcess instanceCousr01cProcess;
	
	@Override
	public void run(ExecutionController ctrl) {
		instanceCousr01cProcess.mainPara((Cousr01cContext) getContext(), ctrl);
	}	
